...
<head>
    ...
    <title>Order #<?php echo htmlspecialchars($order_id); ?></title>
...
<body class="flex flex-col min-h-screen">
    ...
    <main class="flex-grow container mx-auto p-6 md:p-8">
        <div class="content-section max-w-2xl mx-auto">
            <h1 class="text-3xl font-bold mb-2 text-gray-800">Order #<?php echo htmlspecialchars($order['order_id']); ?></h1>
            <p class="text-lg text-gray-600 mb-6">Placed on <?php echo htmlspecialchars(date('F j, Y, g:i a', strtotime($order['created_at']))); ?></p>
            
            <div class="bg-gray-100 p-4 rounded-lg mb-6">
                <p class="font-semibold text-lg">Order Status: <span class="text-green-600"><?php echo htmlspecialchars(ucfirst($order['status'])); ?></span></p>
                <!-- UPDATED: Changed '$' to '₱' for Total Amount -->
                <p class="font-semibold text-lg">Total Amount: <span class="text-gray-900">₱<?php echo htmlspecialchars(number_format($order['total_amount'], 2)); ?></span></p>
            </div>
            ...
            <div class="space-y-4">
                <?php foreach ($order_items as $item): ?>
                    <div class="flex items-center space-x-4 border-b pb-4">
                        <img src="<?php echo htmlspecialchars($item['image_url'] ?? 'https://via.placeholder.com/80.png?text=Product'); ?>" 
                             alt="<?php echo htmlspecialchars($item['product_name']); ?>" 
                             class="w-20 h-20 object-cover rounded-lg">
                        <div class="flex-grow">
                            <p class="font-semibold text-lg"><?php echo htmlspecialchars($item['product_name']); ?></p>
                            <p class="text-gray-600">Quantity: <?php echo htmlspecialchars($item['quantity']); ?></p>
                            <!-- UPDATED: Changed '$' to '₱' for Price Each -->
                            <p class="text-gray-600">Price Each: ₱<?php echo htmlspecialchars(number_format($item['price_each'], 2)); ?></p>
                        </div>
                        <!-- UPDATED: Changed '$' to '₱' for Item Subtotal -->
                        <span class="font-bold text-xl">₱<?php echo htmlspecialchars(number_format($item['price_each'] * $item['quantity'], 2)); ?></span>
                    </div>
                <?php endforeach; ?>
            </div>
